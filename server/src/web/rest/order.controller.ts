...
export class OrderController {
    ...
    async getInventory(@Req() req: Request, @Body() orderDTO: OrderDTO): Promise<Record<string, any> | ApiResponseDTO> {
        await this.orderService.save(orderDTO, req.user?.userName);
        const resp = {
            SOLD: 0,
            PENDING: 0,
            AVAILABLE: 0,
        ...
        orderDTOs.map(orderDTO1 => {
            resp.SOLD += orderDTO1.quantity || 1;
        });
        return resp;
    }
